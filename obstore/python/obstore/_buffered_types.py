...
class ReadableFile(Protocol):
    ...
    def seek(self, offset: int, whence: int = ..., /) -> int:
        ...
        - [`os.SEEK_SET`][] or 0: start of the stream (the default); `offset` should be
            zero or positive
...
# Note: the public API exposes a Protocol, not the literal class exported from
# Rust because we don't want users to rely on nominal subtyping.
class WritableFile(AbstractContextManager, Protocol):
...
# Note: the public API exposes a Protocol, not the literal class exported from
# Rust because we don't want users to rely on nominal subtyping.
class AsyncWritableFile(AbstractAsyncContextManager, Protocol):
